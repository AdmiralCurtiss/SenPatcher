...
bool TryApply(const SenPatcher::GetCheckedFileCallback& getCheckedFile,
    ...
    try {
        ...
        {
            auto d = SenLib::DecompressFromBuffer(PatchData_ed8m2150, PatchLength_ed8m2150);
            if (!d) {
                return false;
            }
        ...
        {
            auto d = SenLib::DecompressFromBuffer(PatchData_ed8m4097, PatchLength_ed8m4097);
            if (!d) {
                return false;
            }
            result.emplace_back(std::move(*d),
        ...
        // Olivier lute sound
        {
            auto d = SenLib::DecompressFromBuffer(PatchData_ed8m4217, PatchLength_ed8m4217);
            if (!d) {
                return false;
            }
            result.emplace_back(std::move(*d),
        ...
        {
            auto d = SenLib::DecompressFromBuffer(PatchData_pc8v02551_4, PatchLength_pc8v02551_4);
            if (!d) {
                return false;
            }
            result.emplace_back(std::move(*d),
        ...
        {
            auto d = SenLib::DecompressFromBuffer(PatchData_pc8v10286_15, PatchLength_pc8v10286_15);
            if (!d) {
                return false;
            }
            result.emplace_back(std::move(*d),
