...
extern "C" {
__declspec(dllexport) char SenPatcherHook[] = "Sen5";
...
static constexpr std::array<std::string_view, 2> s_PkaGroupPrefixes{
    {"data/asset/d3d11/", "data/dlc/asset/d3d11/"}};
...
static void* SetupHacks(HyoutaUtils::Logger& logger) {
...

    {
        ...
        if (settingsFile.IsOpen()) {
            ...
            if (ini.ParseFile(settingsFile)) {
                ...
                check_boolean(
                    "Reverie", "AllowMissingDlcSaveFileLoad", allowMissingDlcSaveFileLoad);
    ...
    if (assetFixes) {
        assetCreationSuccess = SenLib::Sen5::CreateAssetPatchIfNeeded(logger,
                                                                      baseDirUtf8,
                                                                      s_LoadedVanillaP3As,
                                                                      s_LoadedPkaData,
                                                                      s_PkaGroupPrefixes,
    ...
    Align16CodePage(logger, patchExecData.Codespace);
    SenLib::Sen5::InjectAtDecompressPkg(patchExecData, &DecompressPkgForwarder);
    Align16CodePage(logger, patchExecData.Codespace);
