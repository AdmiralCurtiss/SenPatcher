...
namespace SenLib.Sen2.FileFixes {
	...
	public class text_dat_us_t_notecook_tbl : FileMod {
		...
		public IEnumerable<FileModResult> TryApply(FileStorage storage) {
			var s = storage.TryGetDuplicate(new HyoutaUtils.Checksum.SHA1(0x42419bfb7e473d17ul, 0x7b08871627618050ul, 0x0c5d76f5u));
			if (s == null) {
				return null;
			}
			...
			foreach (TblEntry entry in tbl.Entries) {
				if (entry.Name == "QSCook") {
					...
					item.item1line1 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item1line1);
					item.item1line2 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item1line2);
					item.item2line1 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item2line1);
					item.item2line2 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item2line2);
					item.item3line1 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item3line1);
					item.item3line2 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item3line2);
					item.item4line1 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item4line1);
					item.item4line2 = text_dat_us_t_item_tbl.FixHpEpCpSpacing(item.item4line2);
